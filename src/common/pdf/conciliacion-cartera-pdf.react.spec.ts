import { generarPdfConciliacionCarteraReactPdf } from './conciliacion-cartera-pdf.react';
...
describe('generarPdfConciliacionCarteraReactPdf', () => {
  it('resuelve a bytes que empiezan con %PDF-', async () => {
    const bytes = await generarPdfConciliacionCarteraReactPdf(
      makeReporte(),
      makeCopropiedad(),
    );
  ...
  it('agrega texto de alerta cuando diferencia no es cero, produciendo mas bytes', async () => {
    const conciliado = await generarPdfConciliacionCarteraReactPdf(
      makeReporte({ diferencia: 0 }),
      makeCopropiedad(),
    );
    const noConciliado = await generarPdfConciliacionCarteraReactPdf(
  ...
  it('agrega la tabla de anticipos pendientes cuando hay filas, produciendo más bytes', async () => {
    const sinAnticipos = await generarPdfConciliacionCarteraReactPdf(
      makeReporte({ anticiposPendientes: [] }),
      makeCopropiedad(),
    );
    const conAnticipos = await generarPdfConciliacionCarteraReactPdf(
      makeReporte({
        anticiposPendientes: [
          {
            inmuebleCodigo: '502',
